                                  ...
                                  uct_completion_t *comp, int direction)
{
    ...
    if (current_ctx == NULL) {
        ...
        status = uct_cuda_ipc_get_ctx_rscs(iface, current_ctx, &ctx_rsc);
        if (UCS_OK != status) {
    ...
    status = uct_cuda_ipc_get_queue_desc(ctx_rsc, key->dev_num, &q_desc);
    if (status != UCS_OK) {
        return UCS_ERR_IO_ERROR;
    }
    ...
    cuda_ipc_event    = ucs_mpool_get(&ctx_rsc->cuda_event_desc);
