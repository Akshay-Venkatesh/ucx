...
ucs_status_t uct_tcpcm_ep_set_sock_id(uct_tcpcm_iface_t *iface, uct_tcpcm_ep_t *ep)
{
    ...
    if (iface->sock_id_quota > 0) {
        ...
        ep->sock_id_ctx->sock_id = socket(dest_addr->sa_family, SOCK_STREAM, 0);
        if (-1 == ep->sock_id_ctx->sock_id) {
            goto out_free;
        }
...
static UCS_CLASS_INIT_FUNC(uct_tcpcm_ep_t, const uct_ep_params_t *params)
{
    ...
    status = uct_tcp_socket_connect(self->sock_id_ctx->sock_id, sockaddr->addr);
    if (status != UCS_OK) {
        goto err;
    }
