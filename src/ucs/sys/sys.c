...
static ucs_status_t ucs_get_paths(char *dev_loc, char *match, int *num_devices, char **fpaths)
{
    ...
    *num_devices = 0;
    ...
    if (n < 0) {
        ...
        *fpaths = ucs_malloc(sizeof(char) * UCS_FPATH_MAX_LEN * n, "mm_fpaths allocation");
        if (NULL == *fpaths) {
        ...
        for (idx = 0; idx < n; idx++) {
            if (!strncmp(namelist[idx]->d_name, match, strlen(match))) {
                ucs_assert(strlen(namelist[idx]->d_name) <= UCS_FPATH_MAX_LEN);
                dest = (char *) *fpaths + (*num_devices * (sizeof(char) * UCS_FPATH_MAX_LEN));
